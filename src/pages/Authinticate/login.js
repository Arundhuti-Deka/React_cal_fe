...
const Login = () => {
  ...
  const submit = async () => {
    ...
    try {
      const res = await instance.post('/user/authenticate', {
        ...data
      });
      if (res && res.data) {
    ...
    <form onSubmit={handleSubmit} className=" py-6 mx-2">
      <div>
        ...
        {dynamicErrors && dynamicErrors.email &&(
            <p className="text-xs text-red-500 mx-12">{dynamicErrors.email}</p>
          )}
      </div>
      <div>
        <input
        ...
        {errors && errors.password && (
          <p className="text-red-400 md:ml-8">{errors.password}</p>
          )}
          {dynamicErrors && dynamicErrors.password &&(
            <p className="text-xs text-red-500 mx-12">{dynamicErrors.password}</p>
          )}
      </div>
      ...
      <div className="flex justify-center items-center mt-6">
        <button 
         type="submit"
          className="bg-red-500 md:mb-16 md:mt-8 py-2 px-10 text-xl font-sans text-white border
                    focus:outline-none "
        >
          SIGN IN
        </button>
